...
ialex@ISW-240314-1720:~/dokku-deploy$ cat > server_cleanup.sh
#!/usr/bin/env bash
#
# Run a command or script on your server
#
...
check_for_newer_version() {
  ...
  if [[ -f "/pipe.yml" ]]; then
    ...
    pipe_latest_version=$(wget "${wget_debug_level}" -O - "${pipe_repository}"/raw/master/pipe.yml | awk -F ":" '/image/ {print $NF}')
ialex@ISW-240314-1720:~/dokku-deploy$ ./server_cleanup.sh
INFO: Time to Cleanup....
server_cleanup
...
ialex@ISW-240314-1720:~/dokku-deploy$ ls
README.md  dokku_deploy.sh  server_cleanup.sh
ialex@ISW-240314-1720:~/dokku-deploy$ ls
README.md  dokku_deploy.sh  server_cleanup.sh
ialex@ISW-240314-1720:~/dokku-deploy$ cp dokku_deploy.sh hims-middleware.sh
ialex@ISW-240314-1720:~/dokku-deploy$ cat hims-middleware.sh 
#!/usr/bin/env bash
#
# Run a command or script on your server
#
